...
--*/
{
   ...
   //-------自修改
   int baseIdx = fNight ? (256 * 3) : 0;
   for (i = 0; i < 256; i++)
   {
       int idx = baseIdx + i * 3;
       palette[i].r = buf[idx] << 2;
       palette[i].g = buf[idx + 1] << 2;
       palette[i].b = buf[idx + 2] << 2;
   }
   //----------
...
--*/
{
   ...
   */
   ...
   //修改后-------
   memcpy(palette, VIDEO_GetPalette, 256 * sizeof(uint32_t));
   //---------

   //
   // Start fading out...
   //
   time = SDL_GetTicks() + iDelay * 10 * 10; //原来后面60 改成10
...
--*/
{
   ...
   // Start fading in...
   //
   time = SDL_GetTicks() + iDelay * 10 * 10 ;  //原来后面是60 修改到10
   while (TRUE)
   {
      //
      // Set the current palette...
...
--*/
{
   ...
   if (iStep > 0)
   {
      ...
      {
         time = SDL_GetTicks() + 10;  //原来100改成10
   ...
   else
   {
      ...
      {
         time = SDL_GetTicks() + 10; //原来100 改成10
