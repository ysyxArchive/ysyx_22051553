...
static uint32_t color_buf[32 * 32];           //暂存一个block的像素
...
void redraw() {
  ...
  for (y = 0; y < N; y ++) {
    for (x = 0; x < N; x ++) {
      ...
      io_write(AM_GPU_FBDRAW, x * w, y * h, color_buf, w, h, false);      //每次写入一个block
    }
    printf("y is %d\n", y);
  }
  io_write(AM_GPU_FBDRAW, 0, 0, NULL, 0, 0, true);  //同步
}
